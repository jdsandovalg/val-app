...
export default function VotingReportViewerPage() {
  ...
  useEffect(() => {
    setIsClient(true);
    const savedData = localStorage.getItem('votingReportData');
    if (savedData) {
  ...
  const handleShare = async () => {
    ...
    if (!blob) return;
    const fileName = sanitizeFilename(reportData.fileName || 'reporte_votacion.pdf');
    const file = new File([blob], fileName, { type: 'application/pdf' });

    if (navigator.share && navigator.canShare({ files: [file] })) {
      await navigator.share({
        title: t('voting.reportTitle'),
        text: `Reporte de votación del proyecto: ${reportData.projectInfo.descripcion_tarea}`,
        files: [file],
      });
    }
  };
        ...
        {isClient && typeof navigator.share !== 'undefined' && (
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
              <path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.195.025.39.05.588.08a2.25 2.25 0 011.933 2.338V19.5a2.25 2.25 0 01-2.25 2.25h-1.5a2.25 2.25 0 01-2.25-2.25v-2.338c0-.944.52-1.771 1.332-2.15C4.944 12.893 4.5 12.258 4.5 11.5a3 3 0 013-3c.996 0 1.903.406 2.578 1.068c.707-.048 1.43-.09 2.182-.108a3.375 3.375 0 013.375 3.375v1.5c0 .944-.52 1.771-1.332 2.15-.4.15-.82.27-1.264.352M12 15.75a3 3 0 01-3-3M12 15.75a3 3 0 003-3M12 15.75v-1.5" />
            </svg>
          </button>
        )}
        {isClient && typeof navigator.share === 'undefined' && (
