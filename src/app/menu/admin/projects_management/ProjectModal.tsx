              ...
              <Dialog.Panel className={`w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all border-l-4 ${formData.es_propuesta ? 'border-l-blue-500' : 'border-l-yellow-500'}`}>
                ...
                {view === 'selection' && !isEditing && (
                  <div className="mt-4 space-y-3">
                    <button onClick={() => handleTypeSelection('proposal')} className="w-full text-left p-4 rounded-lg shadow-sm transition-all bg-blue-50 hover:bg-blue-100 border-l-4 border-blue-500">
                    ...
                    <button onClick={() => handleTypeSelection('legacy')} className="w-full text-left p-4 rounded-lg shadow-sm transition-all bg-yellow-50 hover:bg-yellow-100 border-l-4 border-yellow-500">
                ...
                {view === 'form' && (
                  <Tab.Group>
                    <Tab.List className="flex space-x-1 rounded-xl bg-gray-900/20 p-1 mt-4">
                      <Tab className={({ selected }) => `w-full rounded-lg py-2.5 text-sm font-medium leading-5 text-gray-700 ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2 ${selected ? 'bg-white shadow' : 'text-gray-500 hover:bg-white/[0.12]'}`}>Información General</Tab>
                      <Tab className={({ selected }) => `w-full rounded-lg py-2.5 text-sm font-medium leading-5 text-gray-700 ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2 ${selected ? 'bg-white shadow' : 'text-gray-500 hover:bg-white/[0.12]'}`}>Detalles y Notas</Tab>
                    </Tab.List>
                    <Tab.Panels className="mt-2">
                      <Tab.Panel className="pt-4 space-y-4 max-h-[60vh] overflow-y-auto pr-2 focus:outline-none">
                        {isEditing && (
